...
\begin{frame}[shrink]\frametitle{DFT和Z变换的关系}%[allowframebreaks][shrink]
$$  \because \quad  X(z) = \sum_{n=-\infty}^{\infty}x(n)z^{-n}\qquad\qquad\quad\quad\quad\quad\quad\quad\quad $$
%我们知道：
\begin{equation*}
\begin{split}
%\because \quad               X(z) &= \sum_{n=-\infty}^{\infty}x(n)z^{-n}\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad \\
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[shrink]\frametitle{几个约定的符号}%[allowframebreaks][shrink]
...
\begin{frame}[shrink]\frametitle{举例说明}%[allowframebreaks][shrink]
...
\begin{frame}\frametitle{线性性质}%[allowframebreaks][shrink]
线性性质很重要
\end{frame}
...
%上式表明将$x(n)$以$ N $为周期进行延拓得到$ \tilde{x}(n) $，再将之左移$ m $位得到$ \tilde{x}(n+m) $，最后取$ \tilde{x}(n) $的主值序列得到$ y(n) $。
...
\begin{frame}[allowframebreaks]\frametitle{ 有限长序列的分解}%[allowframebreaks][shrink]
$3^o$ 讨论：
\begin{enumerate}
...
\begin{frame}[shrink]\frametitle{对称性讨论—共轭对称与共轭反对称序列的DFT}%[allowframebreaks][shrink]
        ...
        \item [(b)]   $\quad x_{op}(n)\leftrightarrow jX_{I}(k)$
            \begin{equation*}
            \begin{split}
           \because \quad  x_{op}(n)  &= \frac{1}{2}\bigg[x(n) - x^{*}(N-n)\bigg] \\%    \quad\quad\textcolor{red}{\bigg(x^*(N-n)\leftrightarrow X^*(k)\bigg)}\\
...
\begin{wenti}
\begin{enumerate}
	\item 那么能否也由频域采样信号$ X(k) $恢复原连续频域函数$ X(e^{j\omega}) $？%原来的信号$ x(n) $，或
	\item 如果可以，其条件是什么？
	\item 内插公式什么形式？
\end{enumerate}
\end{wenti}
...
%\left\{\begin{array}{r@{,\quad}l}
%1  & m= n+l\cdot N \quad\qquad  l\in Z\\
%0     &\mbox{其他m}
%\end{array} \right.
%\end{equation*}
\end{frame}
...
\begin{frame}\frametitle{信号的谱分析}%[allowframebreaks]
\begin{dablock}
...
\end{enumerate}
%\begin{zhuyi}
%直接由分析结果$ X(k) $看不到$ X_a(j\Omega) $的全部频谱特性，只能看到N个离散采样点的谱线，这就是所谓的栅栏效应。
%\end{zhuyi}
...
\begin{frame}[shrink]\frametitle{举例说明：$ X(k) $见板书}%[allowframebreaks]
%    例： 
